<script lang="ts">
	...
	const copy = {
		id: {
			...
			paidStamp: "Lunas",
			failed: "Negatif ditolak", failedBody: "Eksposur gagal — pembayaran dibatalkan. Silakan ulangi dari awal.",
			failedStamp: "Ditolak",
			...
			sPaid: "Lunas", sFailed: "Dibatalkan", sPending: "Menunggu",
			missingOrder: "Nomor seri tidak ditemukan di tautan ini.",
			invalidOrder: "Nomor seri tidak valid.",
			checkFailed: "Gagal memeriksa status pembayaran. Coba cek ulang.",
		...
		en: {
			...
			paidStamp: "Settled",
			failed: "Negative rejected", failedBody: "The exposure failed — payment was cancelled. Please start again.",
			failedStamp: "Rejected",
			...
			sPaid: "Settled", sFailed: "Rejected", sPending: "Pending",
			missingOrder: "No serial number found in this link.",
			invalidOrder: "Invalid serial number.",
			checkFailed: "Could not verify payment status. Please re-check.",
	...
	const ORDER_ID_RE = /^ORDER-[A-Za-z0-9-]+$/;
	...
	const errorText = $derived(
		errorMsg ??
			(orderIdError() === "missing" && status === "failed"
				? t.missingOrder
				: orderIdError() === "invalid" && status === "failed"
					? t.invalidOrder
					: null)
	);
	...
	async function checkOnce() {
		...
		try {
			...
			status = "failed";
			errorMsg = e instanceof Error && e.message ? `${t.checkFailed} (${e.message})` : t.checkFailed;
		} finally {
			checking = false;
		}
	}
			...
			if (!gotResponse) {
				...
				errorMsg =
					lastError instanceof Error && lastError.message
						? `${t.checkFailed} (${lastError.message})`
						: t.checkFailed;
	...
	const heading = $derived(isPaid ? t.paid : isFailed ? t.failed : t.pending);
	const body = $derived(isPaid ? t.paidBody : isFailed ? t.failedBody : t.pendingBody);
	const statusLabel = $derived(isPaid ? t.sPaid : isFailed ? t.sFailed : t.sPending);
	const statusColor = $derived(isPaid ? "text-safelight" : isFailed ? "text-fg-muted" : "text-safelight/80");
	const statusDot = $derived(isPaid ? "bg-safelight" : isFailed ? "bg-fg-muted" : "bg-safelight/70");
...
>
	...
	<article class="proof-slip proof-row relative w-full overflow-hidden rounded-md border border-hair shadow-[0_50px_120px_-50px_rgba(0,0,0,0.8)]">
		...
		<div class="relative px-6 py-7 sm:px-9 sm:py-9">
			...
			{#if errorText}
				<p role="alert" class="proof-row mx-auto mt-3 max-w-sm font-mono text-xs leading-relaxed text-fg-muted">
					{errorText}
				</p>
			{/if}
